...
/**
 * The FAQ, in one place so the accordion the visitor reads and the FAQPage
 * structured data the search engines and AI assistants read are always the
 * same words. The pricing answer is built from the pricing/plans values so it
 * can never drift from what a buyer is actually charged.
 */
export const FAQS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: "About five minutes, and most of that is typing in your first few deals. There is nothing to configure because there are no settings to configure. Create your account, name your team, start adding leads. No consultant, no onboarding call, no administrator to hire.",
  },
  {
  ...
  {
    q: "What does it cost?",
    a: `$${PRICE} per user per month, flat. One rep or a whole team, the same price, and everything is included, so there is no upgrade holding the good part hostage. You get ${TRIAL_DAYS} days free first and we do not ask for a card to start.`,
  },
];
